...
func NewSsdb(host string, port string) (*Client, error) {
	WriteLog("info", "init ssdb %v:%v", host, port)
	iPort, _ := strconv.Atoi(port)
	db, err := Connect(host, iPort)
	if err != nil {
		WriteLog("error", "failed to connect ssdb:%v\n", err)
		return nil, errors.New("failed to connect ssdb")
	}
...
func (c *Client) send(args []interface{}) error {
	...
	buf.WriteByte('\n')
	if c == nil || c.sock == nil {
		return fmt.Errorf("invalid ssdb socket %v", c)
	}
