...
func NewClientCodec(conn io.ReadWriteCloser, packagerName string) ClientCodec {
...
func (c *clientCodec) WriteRequest(r *Request, param interface{}) error {
	c.mutex.Lock()
...
func NewClient(conn io.ReadWriteCloser, packagerName string) *Client {
	return NewClientWithCodec(NewClientCodec(conn, packagerName))
}

// Dial connects to a Yar-RPC server at the specified network address.
func Dial(network, address string, packagerName string) (*Client, error) {
...
func DialTimeout(network, address string, timeout time.Duration, packagerName string) (*Client, error) {
